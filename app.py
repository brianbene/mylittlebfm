...
def analyze_benedicks_portfolio(file):
    ...
    try:
        ...
        for _, row in filtered_data.iterrows():
            ...
            try:
                balance = float(str(row.iloc[16]).replace(', key=lambda x: x['Balance'], reverse=True)
...
# Footer
st.markdown("---")
st.markdown('<div style="text-align: center; opacity: 0.7;"><p>🚀 My Little BFM • Enhanced with Smart APPN Charging, Expiry Analysis & PM Analysis</p></div>', unsafe_allow_html=True), '').replace(',', '').strip() or 0)
